...
function resolveClaudePath(): string {
  try {
    ...
    throw new Error("Cannot find 'claude' on PATH. Pass --claude-path or add it to your PATH.");
  }
}
...
async function main(): Promise<void> {
  ...
  program
    .name('two-claudes')
    .description('Two Claude instances talking to each other (driven by claude-inject SDK)')
    ...
    .option('--claude-path <path>', 'Path to claude CLI')
    .parse(process.argv);
  ...
  // files and its sessions don't accumulate alongside the source project.
  ...
  const config: LoopConfig = {
    ...
    cwdA,
    cwdB,
    toolsA: !(opts.director || opts.builder), // Director/Builder are text-only
    skipPermissionsB: opts.director || opts.builder,
